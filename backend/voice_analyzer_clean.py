    ...
    try:
        ...
        if audio_data is not None and len(audio_data) > 0:
            ...
            try:
                ...
                f0_valid = np.array([])  # Initialize for later use
            ...
            try:
                if len(f0_valid) > 10:  # Need enough samples for jitter calculation
                    # Calculate jitter (frequency variation)
                    f0_diff = np.diff(f0_valid)
                    jitter = np.mean(np.abs(f0_diff)) / np.mean(f0_valid)
                    features['mdvpJitter'] = float(min(max(jitter, 0.003), 0.02))
                    ...
                    if len(rms) > 10:
                        ...
                        shimmer = np.mean(np.abs(rms_diff)) / np.mean(rms)
                        features['mdvpShimmer'] = float(min(max(shimmer, 0.02), 0.08))
                    
                    logger.info(f"Jitter/Shimmer extracted: Jitter={features['mdvpJitter']:.4f}, Shimmer={features['mdvpShimmer']:.4f}")
            except Exception as e:
                logger.warning(f"Error extracting jitter/shimmer: {e}. Using default values.")
            ...
            try:
                ...
                if len(spectral_centroids) > 0:
                    features['spread1'] = float(np.mean(spectral_centroids) / 1000.0 - 6.5)  # Normalize
                    features['spread2'] = float(np.std(spectral_centroids) / 1000.0)
                    
                # Calculate DFA-like measure using spectral features
                if len(spectral_rolloff) > 0:
                    features['dfa'] = float(min(max(np.std(spectral_rolloff) / np.mean(spectral_rolloff), 0.6), 0.8))
                    
                logger.info(f"Spectral features extracted: spread1={features['spread1']:.2f}, spread2={features['spread2']:.2f}, dfa={features['dfa']:.2f}")
            except Exception as e:
                logger.warning(f"Error extracting spectral features: {e}. Using default values.")
            
            # 5. Attempt to extract PPE (Pitch Period Entropy)
            try:
                if 'mdvpFo' in features and features['mdvpFo'] > 0 and len(f0_valid) > 5:
                    # Calculate periods from frequency
                    periods = 1.0 / f0_valid
                    
                    # Calculate entropy
                    hist, _ = np.histogram(periods, bins=min(10, len(periods)//2), density=True)
                    if len(hist) > 0:
                        ppe = entropy(hist + 1e-10)  # Add small value to avoid log(0)
            ...
            # 6. Extract RPDE-like measure
            try:
                # Use spectral entropy as a proxy for RPDE
                stft = librosa.stft(audio_data)
                magnitude = np.abs(stft)
                spectral_entropy = entropy(np.mean(magnitude, axis=1) + 1e-10)
                features['rpde'] = float(min(max(spectral_entropy / 10.0, 0.4), 0.6))
                logger.info(f"RPDE-like feature extracted: {features['rpde']:.2f}")
            except Exception as e:
                logger.warning(f"Error extracting RPDE feature: {e}. Using default value.")
            
            # 7. Extract D2 (correlation dimension approximation)
            try:
                # Use autocorrelation as a proxy for D2
